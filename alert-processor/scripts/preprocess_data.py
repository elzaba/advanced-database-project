...
def resample_data(df):
    ...
    # Apply SMOTE for oversampling the minority classes and RandomUnderSampler for undersampling the majority class
    smote = SMOTE(sampling_strategy="auto", random_state=42)  # Oversample minority classes
    X_res, y_res = smote.fit_resample(X_imputed, y)

    # Optionally, undersample the majority class to balance the data further (if required)
...
def preprocess_data(raw_data, thresholds, span=10):
    ...
    for metric_name, data in raw_data.items():
        try:
            ...
            # Labeling: 0 = Noise, 1 = Warning Alert, 2 = Critical Alert
            df["label"] = 0  # Default to Noise
            df.loc[(df["value"] > warning_threshold) & (df["value"] <= critical_threshold), "label"] = 1
            df.loc[df["value"] > critical_threshold, "label"] = 2
