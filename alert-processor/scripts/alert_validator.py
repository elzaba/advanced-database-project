...
def forward_to_alertmanager(alert, severity):
    """Forward validated alerts to Alertmanager."""
    try:
        ...
        response.raise_for_status()
        logging.info(f"Alert '{alert['labels']['alertname']}' forwarded to Alertmanager.")
        ...
        for alert in alerts:
            if alert["labels"]["alertname"] in metric["alert_names"]:
                ...
                if severity != "noise":  # Forward only warning or critical alerts
                    forward_to_alertmanager(alert, severity)
